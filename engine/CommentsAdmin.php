...
########################################
class CommentsAdmin extends Engine
{
  ...
  function fetch()
  {
    ...
    {
    	$filter['type'] = $type;
 		$this->templates->assign('type', $type);
 	}
	...
	foreach($this->products->get_products(array('id'=>$products_ids)) as $p)
 	...
 	$this->templates->assign('pages_count', ceil($comments_count/$filter['limit']));
 	$this->templates->assign('current_page', $filter['page']);

 	$this->templates->assign('comments', $comments);
 	$this->templates->assign('comments_count', $comments_count);

	return $this->templates->fetch('comments.tpl');
  }
}
