...
############################################
class OrderAdmin extends Engine
{
	public function fetch()
	{
			...
			if($order->id)
			{
				...
				foreach($purchases as $purchase)
				{
					...
					elseif(!$purchase->id = $this->orders->add_purchase(array('order_id'=>$order->id, 'variant_id'=>$purchase->variant_id, 'variant_name'=>$variant->name, 'price'=>$purchase->price, 'amount'=>$purchase->amount)))
						$this->templates->assign('message_error', 'error_closing');
				...
				if($new_status == 0)					
				{
					if(!$this->orders->open(intval($order->id)))
						$this->templates->assign('message_error', 'error_open');
					else{
						$this->orders->update_order($order->id, array('status'=>0));
						$this->templates->assign('message_success', 'status_updated');
					}
				}
				elseif($new_status == 1)					
				{
		...
		$this->templates->assign('purchases_count', $purchases_count);
		$this->templates->assign('subtotal', $subtotal);
		...
		if(!empty($order->id))
		{
			...
			{
				$this->templates->assign('payment_method', $payment_method);
		
				// Валюта оплаты
				$payment_currency = $this->money->get_currency(intval($payment_method->currency_id));
				$this->templates->assign('payment_currency', $payment_currency);
			}
			// Пользователь
			if($order->user_id)
				$this->templates->assign('user', $this->users->get_user(intval($order->user_id)));
	
			// Соседние заказы
			$this->templates->assign('next_order', $this->orders->get_next_order($order->id, $this->request->get('status', 'string')));
			$this->templates->assign('prev_order', $this->orders->get_prev_order($order->id, $this->request->get('status', 'string')));
		}

		// Все способы доставки
		/* $deliveries = $this->delivery->get_deliveries(); */
		$this->templates->assign('deliveries', $deliveries);
		...
		// Метки заказов
	 	...
	 	$this->templates->assign('labels', $labels);
	  	
	 	$this->templates->assign('order_labels', $order_labels);	  	
		
		if($this->request->get('view') == 'print')
 		  	return $this->templates->fetch('products/order_print.tpl');
 	  	else
	 	  	return $this->templates->fetch('products/order.tpl');
	}
}