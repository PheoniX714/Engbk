...
class PaymentMethodAdmin extends Engine
{	
	...
	{	
		...
		if($this->request->method('post'))
		{
			...
			elseif(empty($payment_method->id))
			{
  				...
  				$this->templates->assign('message_success', 'Добавлено');
	    	}
	    	else
	    	{
	    		$this->payment->update_payment_method($payment_method->id, $payment_method);
  				$this->templates->assign('message_success', 'Обновлено');
	    	}
		...
		$this->templates->assign('payment_settings', $payment_settings);
		$payment_modules = $this->payment->get_payment_modules();
		$this->templates->assign('payment_modules', $payment_modules);
		
		$currencies = $this->money->get_currencies();
		$this->templates->assign('currencies', $currencies);
		
		
  	  	return $this->templates->fetch('settings/payment_method.tpl');
	}
