...
class ExportAjax extends Engine
{	
		...
		$products = array();
 		foreach($this->products->get_products(array('page'=>$page, 'limit'=>$this->products_count)) as $p)
 		{
			...
			if($p->brand_id == null)
 			...
 			$translation = $this->products->get_product_translation(intval($p->id), intval($language->id));
			$products[$p->id] = array_merge((array)$products[$p->id], (array)$translation);
