...
########################################
class OrdersAdmin extends Engine
{
	public function fetch()
	{
		...
		// Фильтр по метке
	  	...
	  	if(!empty($label))
	  	{
		 	...
		 	$this->templates->assign('label', $label);
		}
		...
		if(empty($keyword))
		{
			...
			$filter['status'] = $status;
		 	$this->templates->assign('status', $status);
		}
		...
		$orders_labels = array();
	 	...
	 	$this->templates->assign('orders_count', $orders_count);
	
	 	$this->templates->assign('orders', $orders);
	
		// Метки заказов
	  	$labels = $this->orders->get_labels();
	 	$this->templates->assign('labels', $labels);
	  	
		return $this->templates->fetch('products/orders.tpl');
	}
}
