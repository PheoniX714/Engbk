...
class ProductsImportAdmin extends Engine
{	
	public $import_files_dir = 'engine/files/import/';
	...
	public function fetch()
	{
		...
		{
			$this->templates->assign('message_error', 'locale_error');
			$this->templates->assign('locale', $this->locale);			
		}
		...
		if($this->request->method('post') && ($this->request->files("file")))
		{
			...
			if(!move_uploaded_file($uploaded_name, $temp))
				$this->templates->assign('message_error', 'upload_error');
			
			if(!$this->convert_file($temp, $this->import_files_dir.$this->import_file))
				$this->templates->assign('message_error', 'convert_error');
			else
				$this->templates->assign('filename',  $this->request->files("file", "name"));
			unlink($temp);	
		}

		return $this->templates->fetch('products/import.tpl');
	}
