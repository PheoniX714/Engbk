...
class Image extends Engine
{
	...
	public function upload_image($filename, $name, $path = '', $uploaded = true)
	{
		// Имя оригинального файла
		...
		{			
			while(file_exists($this->config->root_dir.$path.$new_name))
			{	
				$new_base = pathinfo($new_name, PATHINFO_FILENAME);
				if(preg_match('/_([0-9]+)$/', $new_base, $parts))
					$new_name = $base.'_'.($parts[1]+1).'.'.$ext;
			...
			if($uploaded)
				if(move_uploaded_file($filename, $this->config->root_dir.$path.$new_name))			
					return $new_name;
			else
				if(rename($filename, $this->config->root_dir.$path.$new_name))			
					return $new_name;
		}
