...
class Comments extends Engine
{
	...
	{
		$query = $this->db->placehold("SELECT c.id, c.object_id, c.name, c.rating, c.ip, c.type, c.text, c.date, c.approved FROM __comments c WHERE id=? LIMIT 1", intval($id));
	...
	{
		$query = $this->db->placehold("SELECT a.id, a.comment_id, a.text FROM __answers a WHERE comment_id=? LIMIT 1", intval($id));
	...
	{	
		...
		$query = $this->db->placehold("SELECT c.id, c.object_id, c.ip, c.name, c.rating, c.text, c.type, c.date, c.approved
	...
	{	
		$query = $this->db->placehold('INSERT INTO __answers
		SET ?%',
		$answer);
	...
	{
		$query = $this->db->placehold("UPDATE __answers SET ?% WHERE id in(?@) LIMIT 1", $answer, (array)$id);
		$this->db->query($query);
		return $id;
	}
