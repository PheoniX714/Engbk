...
class CartView extends View
{
  ...
  public function __construct()
  {
    ...
    if(isset($_POST['checkout']))
    {
		...
		$this->templates->assign('delivery_id', $order->delivery_id);
		$this->templates->assign('name', $order->name);
		$this->templates->assign('email', $order->email);
		$this->templates->assign('phone', $order->phone);
		$this->templates->assign('address', $order->address);
		$this->templates->assign('comment', $order->comment);
    	...
    	{
    		$this->templates->assign('error', 'empty_phone');
    	}
	    ...
	    if($amounts = $this->request->post('amounts'))
	    {
	    	...
	    	else
	    	{
				...
				if(empty($coupon) || !$coupon->valid)
				{
					...
					$this->templates->assign('coupon_error', 'invalid');
				}
				else
				{
					$this->cart->apply_coupon($coupon_code);
	...
	{  
		...
		if($this->user)
		{
			...
			if($last_order)
			{
				$this->templates->assign('name', $last_order->name);
				$this->templates->assign('email', $last_order->email);
				$this->templates->assign('phone', $last_order->phone);
				$this->templates->assign('address', $last_order->address);
			}
			else
			{
				$this->templates->assign('name', $this->user->name);
				$this->templates->assign('email', $this->user->email);			
			}
		}
		
		// Если существуют валидные купоны, нужно вывести инпут для купона
		/* if($this->coupons->count_coupons(array('valid'=>1))>0)
			$this->templates->assign('coupon_request', true); */

		// Выводим корзину
		return $this->templates->fetch('cart.tpl');
	}
